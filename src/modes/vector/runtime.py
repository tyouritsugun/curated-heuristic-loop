...
from src.search.thread_safe_faiss import (
...
class VectorOperationsAdapter:
    """Operations adapter for vector-capable mode."""
...
def _build_embedding_stack(config: Any, db: Any) -> tuple[
    ...
    try:
        ...
        try:
            ...
            logger.info("✓ Embedding client loaded successfully: %s", config.embedding_model)
        except Exception as exc:
            logger.error("✗ Embedding client initialization failed: %s", exc, exc_info=True)
        ...
        if embedding_client:
            try:
                ...
                logger.error("✗ FAISS initialization failed with exception: %s", exc, exc_info=True)
        ...
        if embedding_client and thread_safe_faiss:
            try:
                ...
                logger.error("✗ Vector provider initialization failed: %s", exc, exc_info=True)
...
def build_runtime(config: Any, db: Any, worker_control_service: Any) -> ModeRuntime:
    """Build runtime components for vector/semantic-capable mode."""
    embedding_client, thread_safe_faiss, reranker_client, vector_provider = _build_embedding_stack(
        config, db
    )
    ...
    try:
        ...
        logger.info("✓ Search service initialized with primary provider: %s", primary_provider)
    except Exception as exc:
        logger.error("✗ Search service initialization completely failed: %s", exc, exc_info=True)
        logger.warning("Search service initialization failed: %s", exc)
        search_service = None

    background_worker, worker_pool = _build_worker_stack(
        config, db, worker_control_service, embedding_client, thread_safe_faiss
    )
